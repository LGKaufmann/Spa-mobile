...
export const LoginPersonalApp = () => {
  ...
  useEffect(() => {
    if (user && !loading) {
      // Redirige a LandingAdminApp después de iniciar sesión
      navigate("/adminApp"); // Cambia esto si "adminApp" no es la ruta correcta para LandingAdminApp
    }
  ...
  return (
    <main className="flex items-center justify-center h-screen mx-auto relative">
      ...
      <div className="absolute top-4 left-4">
        <button
          onClick={() => navigate("/")}
          className="mb-6 bg-[#cb0c4f] text-white py-2 px-4 rounded hover:bg-[#a90b3f]"
        >
          Volver al inicio
        </button>
      </div>
